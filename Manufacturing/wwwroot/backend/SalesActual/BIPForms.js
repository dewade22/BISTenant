...
function MonthlyRevenueBIP(myDate, result) {
    ...
    for (let i = 0; i < sales.length; i++) {
        let product = `<tr>
                        ...
                        <td>${sales[i].revenueMonth == 0 ? `Rp. ${(sales[i].revenueMonth).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `<a href="${baseurl}/Balimoon/SalesActual/MonthlySalesBIP?dateTime=${dates}&category=${sales[i].itemCategory}">Rp. ${(sales[i].revenueMonth).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</a>`}</td>
    ...
    let rowTotal = `<tr>
                        ...
                        <td style="font-weight:bold">${totalToday == 0 ? `RP. ${(totalToday).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `<a href="${baseurl}/Balimoon/SalesActual/TodaySalesBIP?dateTime=${dates}&category=">Rp. ${(totalToday).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</a>`}</td>
                        <td style="font-weight:bold">${totalBulan == 0 ? `Rp. ${(totalBulan).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `<a href="${baseurl}/Balimoon/SalesActual/MonthlySalesBIP?dateTime=${dates}&category=">Rp. ${(totalBulan).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</a>`}</td>
