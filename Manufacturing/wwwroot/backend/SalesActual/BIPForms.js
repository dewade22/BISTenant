...
function Days(date = fullDT) {
    ...
    $.ajax({
        ...
        url: baseurl + '/BalimoonBIP/SalesActual/BIPDaysMonth',
...
function insertSalesActual(date, res) {
    ...
    $.ajax({
        ...
        url: baseurl + '/BalimoonBIP/SalesActual/BMISalesAndTarget',
        success: function (data) {
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                jQuery("#tableBIPMonth tbody").append(isi);
            ...
            jQuery("#tableBIPMonth tbody").append(isitabel);
...
function insertSalesActualYearly(date, days, datas) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        jQuery("#tableBIPYear tbody").append(products);
    ...
    jQuery("#tableBIPYear tbody").append(addLandedCost);
    jQuery("#tableBIPYear tbody").append(pemisah);
    //Tambah Total
    let rowTotal = `<tr>
                        <td style="font-weight:bold">Total</td>
...
function insertSalesActualYearlyLiters(date, days, datas) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        jQuery("#tableBIPYearlyLiters tbody").append(products);
...
function insertSummaryRevenue(date, days, MonthlySales, MonthlyLiters) {
    let Data = `<tr>
                    <td rowspan="2">BIP : ${days.noDaysM} Of ${days.dayM} Days</td>
...
function insertSummaryRevenueYearly(date, days, YearlySales, YearlyLiters) {
    let Data = `<tr>
                    <td rowspan="2">BIP : ${days.noDaysY} Of ${days.dayY}</td>
