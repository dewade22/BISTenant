...
function Days(date = fullDT) {
    ...
    $.ajax({
        ...
        url: baseurl + '/BalimoonBMI/SalesActual/SalesAndTarget',
        success: function (result) {
            ...
            let monthlyRevenue = MonthlyRevenueBMI(myDate, result);
            let monthlyLiters = MonthlyLitersBMI(myDate, result);
            let yearlyRevenue = YearlyRevenueBMI(myDate, result);
            let yearlyLiters = YearlyLitersBMI(myDate, result);
...
function MonthlyRevenueBMI(myDate, result) {
    ...
    for (let i = 0; i < sales.length; i++) {
        let product = `<tr>
                        ...
                        <td>${sales[i].revenueDay == 0 ? `RP. ${(sales[i].revenueDay).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `<a href="${localStorage.getItem('thisAddress')}/SalesActual/TodaySalesBMI?dateTime=${dates}&category=${sales[i].itemCategory}">Rp. ${(sales[i].revenueDay).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</a>`}</td>
                        <td>${sales[i].revenueMonth == 0 ? `Rp. ${(sales[i].revenueMonth).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `<a href="${localStorage.getItem('thisAddress')}/SalesActual/MonthlySalesBMI?dateTime=${dates}&category=${sales[i].itemCategory}">Rp. ${(sales[i].revenueMonth).toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2})}</a>`}</td>
        ...
        jQuery("#tableBMIMonth tbody").append(product);
    ...
    jQuery("#tableBMIMonth tbody").append(totalLC);

    jQuery("#tableBMIMonth tbody").append(pemisah);
    //Total
    let rowTotal = `<tr>
                        <td style="font-weight:bold">Total</td>
                        <td style="font-weight:bold">${totalToday == 0 ? `Rp. ${(totalToday).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `<a href="${localStorage.getItem('thisAddress')}/SalesActual/TodaySalesBMI?dateTime=${dates}&category=">Rp. ${(totalToday).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</a>`}</td>
                        <td style="font-weight:bold">${totalBulan == 0 ? `Rp. ${(totalBulan).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : `<a href="${localStorage.getItem('thisAddress')}/SalesActual/MonthlySalesBMI?dateTime=${dates}&category=">Rp. ${(totalBulan).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</a>`} </td>
...
function MonthlyLitersBMI(myDate, result) {
...
function YearlyRevenueBMI(myDate, result) {
    ...
    for (let i = 0; i < sales.length; i++) {
        ...
        jQuery("#tableBMIYear tbody").append(product);
        totalBulan += sales[i].revenueMonth;
    ...
    jQuery("#tableBMIYear tbody").append(totalLC);
...
function YearlyLitersBMI(myDate, result) {
    let Liters = result.salesActual;
    //Untuk dijadikan total
    let totalBulan = 0;
    let totalTahun = 0;
    ...
    for (let i = 0; i < Liters.length; i++) {
        ...
        jQuery("#tableBMIYearlyLiters tbody").append(product);
        totalBulan += Liters[i].litersMonth;
